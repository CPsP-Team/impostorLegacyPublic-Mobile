...
class Logger
{
	...
	{
		#if sys
		if (!FileSystem.exists(#if mobile MobileUtil.getDirectory() + #end folder) && !FileSystem.isDirectory(#if mobile MobileUtil.getDirectory() + #end folder))
		{
			FileSystem.createDirectory(#if mobile MobileUtil.getDirectory() + #end folder);
		}
		...
		try
		{
			File.saveContent(#if mobile MobileUtil.getDirectory() + #end dumpPath, content);
		}
		catch (e) {}
		#end
	}
